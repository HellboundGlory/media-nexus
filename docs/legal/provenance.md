...
| Upstream Cardigann definition sync (`media.definitionSync` job → `indexer_definition` built-ins) | Fetches `definitions/v11/*.yml` from `github.com/Prowlarr/Indexers` at runtime and stores the YAML data | Data integration, not application code | Same owner-granted verbal permission (D4); synced defs are user-visible indexer data, `supported`/`unsupported` tagged, never bundled as app code | Active |
