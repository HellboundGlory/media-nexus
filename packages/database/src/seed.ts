...
export async function seedStatic(db: Db): Promise<void> {
  ...
  const jobs = [
    ...
    { key: "media.collectionSync", name: "Collection sync", description: "Refresh tracked TMDB collections' parts and auto-add missing movies for monitored collections (search when search-on-add is set)", schedule: "0 */6 * * *", timeoutMs: 300_000, maxRetries: 2, priority: 150 },
