  ...
  return (
    <div className="space-y-4">
      <div>
        ...
        <p className="text-sm text-zinc-500">Upcoming air-dated episodes (next 30 days).</p>
      </div>
      {entries.isError ? <ErrorState error={entries.error} onRetry={() => entries.refetch()} /> : entries.data?.length === 0 ? (
        <EmptyState title="Nothing scheduled" hint="Episodes with air dates (future) will show here." />
      ) : (
