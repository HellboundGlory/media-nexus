...
export default function Series() {
  ...
  const [filter, setFilter] = useState(""); // "" | monitored | unmonitored (NO "missing" for series)
  ...
  const series = useInfiniteQuery({
    queryKey: ["series", search, sort, sortDir, filter],
    queryFn: ({ pageParam }) => {
      ...
      return api.get<Paged<SeriesRow>>(`/series?${p.toString()}`);
    },
    initialPageParam: 1,
